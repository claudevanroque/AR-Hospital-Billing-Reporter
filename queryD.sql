WITH PaymentSummary AS (
    SELECT 
        ...
        AR_P.HMOID,
        INS.INSURANCETYPE,
        AR_PD.PHOSPAMOUNT AS HOSPAMOUNT,
        AR_PD.HOSPTAX,
...
SELECT DISTINCT
    ...
    COALESCE(HOS_BILL.TotalPHICAmount, 0) AS ARPHIC,


    HOS_BILL.TotalPHICWH as "ARPHIC (WH)",
    COALESCE(HOS_BILL.TotalActualPayment, 0) as "ACTUAL PHIC",

    COALESCE(AR_PHICPAYMENTDETAIL.Paid, 0) as PHIC_PAID,
    COALESCE(AR_PHICPAYMENTDETAIL.Paid_WH, 0) as "PHIC_WH TAX Source",
    COALESCE(AR_PHICPAYMENTDETAIL.ActualPaid, 0) as "PHIC_ACTUAL PAID",
    COALESCE(HOS_BILL.TotalPHICAmount, 0) - COALESCE(AR_PHICPAYMENTDETAIL.Paid, 0) AS "ARPHIC BALANCE",
    COALESCE(HOS_BILL.TotalPHICWH, 0) - COALESCE(AR_PHICPAYMENTDETAIL.Paid_WH, 0) AS "ARPHIC (WH) BALANCE",
    COALESCE(HOS_BILL.TotalActualPayment, 0) - COALESCE(AR_PHICPAYMENTDETAIL.ActualPaid , 0 ) AS "ACTUAL PHIC BALANCE",
    ...
    COALESCE(HOS_BILL.TotalCompAmount, 0) as ARCOMP,
    COALESCE(HOS_BILL.TotalCompWH, 0) as "ARCOMP (WH)",
    COALESCE(HOS_BILL.TotalActualPaymentComp, 0) as "ACTUAL COMP",
    COALESCE(AR_PD.CompanyAmount, 0) AS COMP_PAID,
    COALESCE(AR_PD.Paid_WH_Company, 0) AS "COMP_WH TAX Source",
    COALESCE(AR_PD.ActualPaid_Company, 0) as "COMP_ACTUAL PAID",
    ROUND(COALESCE(HOS_BILL.TotalCompAmount, 0) - COALESCE(AR_PD.CompanyAmount, 0), 0) AS "COMP BALANCE",
    COALESCE(HOS_BILL.TotalCompWH, 0) - COALESCE(AR_PD.Paid_WH_Company, 0) AS "COMP (WH) BALANCE",
    COALESCE(HOS_BILL.TotalActualPaymentComp, 0) - COALESCE(AR_PD.ActualPaid_Company, 0) AS "ACTUAL COMP BALANCE",
    LTRIM(RTRIM(AR_PD.CompanyRef)) AS COMP_RECEIPT,
    LTRIM(RTRIM(AR_PD.CompanyRemarks)) AS COMP_REMARKS,
    

    AR_PD.HMOName AS "HEALTH ORG",
    COALESCE(HOS_BILL.TotalHMOAmout, 0) as ARHMO,
    COALESCE(HOS_BILL.TotalHMOWH, 0) AS "ARHMO (WH)",
    COALESCE(HOS_BILL.TotalActualPaymentHMO, 0) AS "ACTUAL HMO",
    COALESCE(AR_PD.HMOAmount, 0) AS HMO_PAID,
    COALESCE(AR_PD.Paid_WH_HMO, 0) AS "HMO_WH TAX Source",
    COALESCE(AR_PD.ActualPaid_HMO, 0) as "HMO_ACTUAL PAID",
    ROUND(COALESCE(HOS_BILL.TotalHMOAmout, 0) - (COALESCE(AR_PD.HMOAmount, 0) + COALESCE(AR_PD.Paid_WH_HMO, 0)), 2)  AS "HMO BALANCE",
    COALESCE(HOS_BILL.TotalHMOWH, 0) - COALESCE(AR_PD.Paid_WH_HMO, 0) AS "HMO (WH) BALANCE",
    COALESCE(HOS_BILL.TotalActualPaymentHMO, 0) - COALESCE(AR_PD.ActualPaid_HMO, 0) AS "ACTUAL HMO BALANCE",
